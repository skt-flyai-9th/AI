...
OpenAI를 사용하는 모든 Agent와 Database Knowledge Manager의 기본 모델은 `gpt-5.4-mini`로 통일합니다.
...
모든 숏폼은 촬영 가이드의 장면 순서에 맞춰 `shooting_scene_order`를 전달한다. 각 촬영
영상은 하나의 촬영 컷과 대응하며 AI 서버는 이 순서를 그대로 편집 입력 순서로 사용한다.
