...
class VideoEditingDBRules(BaseModel):
    ...
    # Compact machine-readable strings generated by GPT from Gemini reference
    # evidence. They keep the original-video segment context and effect grammar
    # available to the Editing Agent without changing the relational DB schema.
    reference_segment_guides: list[str] = Field(default_factory=list, max_length=100)
    effect_guides: list[str] = Field(default_factory=list, max_length=100)
